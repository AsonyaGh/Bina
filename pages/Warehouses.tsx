...
export const Warehouses: React.FC = () => {
  ...
  const refreshData = () => {
    setLocations(storageService.getLocations().filter(l => l.type === 'WAREHOUSE'));
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (isEditing && selectedLocation) {
        ...
        storageService.logAction(user?.id || 'system', `Updated warehouse ${name}`);
    } else {
        ...
        storageService.logAction(user?.id || 'system', `Created warehouse ${name}`);
    }
    refreshData();
    setShowModal(false);
  };
