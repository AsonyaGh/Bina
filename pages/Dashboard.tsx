...
import { MotorcycleStatus, Role } from '../types';
...
export const Dashboard: React.FC = () => {
  const { user } = useAuth();
  const bikes = storageService.getMotorcycles();
  ...
  const stockData = useMemo(() => {
    ...
    bikes.forEach(b => statusCounts[b.status]++);
  ...
  const salesTrendData = useMemo(() => {
      ...
      // Since mock sales might be empty, we'll generate dummy trend
      return [
          { name: 'Jan', sales: 4 },
          { name: 'Feb', sales: 7 },
          { name: 'Mar', sales: 5 },
