...
import { Role, TransferStatus, MotorcycleStatus } from '../types';
...
export const Reports: React.FC = () => {
  ...
  const filteredSales = useMemo(() => {
    ...
    if (user.role === Role.SALES_OFFICER) {
      // Sales officers can see sales from their branch (or just their own, depending on policy. 
      // Usually reporting implies seeing branch performance for managers, maybe just own for officers. 
      // Let's allow officers to see branch sales for context).
      return sales.filter(s => s.branchId === user.locationId);
    }
    return [];
  }, [sales, user]);
  ...
  const handleExport = () => {
    // In a real app, this would generate a CSV based on the filtered data
