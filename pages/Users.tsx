...
export const Users: React.FC = () => {
  ...
  const handleSaveUser = (e: React.FormEvent) => {
    ...
    if (isEditing && selectedUser) {
      ...
      storageService.updateUser(updatedUser);
      storageService.logAction(currentUser?.id || 'admin', `Updated user details for ${email}`);
    } else {
      const newUser: User = {
        id: `u_${Date.now()}`,
        name,
      ...
      storageService.logAction(currentUser?.id || 'admin', `Created user ${email}`);
    }
  ...
  const handleResetPassword = (e: React.FormEvent) => {
    ...
    if (selectedUser && password) {
      ...
      storageService.updateUser(updatedUser);
      storageService.logAction(currentUser?.id || 'admin', `Reset password for ${selectedUser.email}`);
