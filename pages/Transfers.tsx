...
export const Transfers: React.FC = () => {
  ...
  const handleCreateTransfer = (e: React.FormEvent) => {
    ...
    // Branch Manager initiating -> PENDING_APPROVAL
    // Admin or Warehouse Manager -> PENDING (Directly In Transit)
    ...
    storageService.createTransfer(newTransfer);
    storageService.logAction(user.id, `Created transfer ${newTransfer.reference} (${initialStatus})`);
    refreshData();
    setShowModal(false);
    setSelectedBikes([]);
  ...
  const handleReceive = (transferId: string) => {
    if (!user) return;
    storageService.completeTransfer(transferId, user.id);
    storageService.logAction(user.id, `Received/Confirmed transfer ${transferId}`);
    refreshData();
  };

  const handleApprove = (transferId: string) => {
      if (!user) return;
      storageService.approveTransfer(transferId, user.id);
      storageService.logAction(user.id, `Approved transfer ${transferId}`);
      refreshData();
  };
  ...
  const handleCancel = (transferId: string) => {
      ...
      if (confirm("Are you sure you want to cancel this transfer? Bikes will be returned to the origin inventory.")) {
          storageService.cancelTransfer(transferId, user.id);
          storageService.logAction(user.id, `Cancelled transfer ${transferId}`);
          refreshData();
      }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4">
        ...
        {relevantTransfers.length === 0 && <p className="text-gray-500">No transfers found.</p>}
      </div>
