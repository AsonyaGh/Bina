...
import { Sale, Motorcycle, MotorcycleStatus, Role, Location } from '../types';
import { useAuth } from '../context/AuthContext';
import { Plus, Info, Edit2, Trash2, X, RefreshCw } from 'lucide-react';
...
export const Sales: React.FC = () => {
  ...
  const updateAvailableBikesForLocation = async (locationId: string) => {
      const allBikes = await storageService.getMotorcycles();
      setAvailableBikes(allBikes.filter(b => 
        b.currentLocationId === locationId && b.status === MotorcycleStatus.AT_BRANCH
      ));
  };
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {loading ? (
                <tr><td colSpan={7} className="p-4 text-center">Loading sales...</td></tr>
              ) : filteredSales.map(s => (
                <tr key={s.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  <td className="px-6 py-4 dark:text-gray-300">{new Date(s.date).toLocaleDateString()}</td>
                  <td className="px-6 py-4 dark:text-gray-300 text-sm">{getLocationName(s.branchId)}</td>
                  ...
                  <td className="px-6 py-4">
                     <div className="flex justify-center gap-2">
                        {canManageSale(s) ? (
                            <>
                                <button onClick={() => openEditModal(s)} className="p-1 text-blue-600 hover:bg-blue-50 rounded"><Edit2 size={16} /></button>
              ...
              {!loading && filteredSales.length === 0 && (
                <tr><td colSpan={7} className="px-6 py-8 text-center text-gray-500">No sales records found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md p-6">
            ...
            <form onSubmit={handleSaleSubmit} className="space-y-4">
              ...
              {user?.role === Role.ADMIN && !isEditing && (
                  <div>
                    <label className="block text-sm font-medium mb-1 dark:text-gray-300">Sales Branch</label>
                    <select 
                        required 
              ...
              {!isEditing && (
                  <div>
                    ...
                    {availableBikes.length > 0 ? (
                      <select required value={selectedBike} onChange={e => setSelectedBike(e.target.value)} className="w-full p-2 border border-gray-600 rounded-lg bg-gray-700 text-white">
                      ...
                      <div className="p-3 bg-yellow-50 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200 rounded-lg text-sm flex items-start">
                        ...
                        <p>{user?.role === Role.ADMIN && !targetBranchId ? 'Please select a branch first.' : 'No motorcycles available for sale at this branch.'}</p>
                      </div>
                    )}
                  </div>
              )}
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button type="submit" className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700" disabled={!isEditing && availableBikes.length === 0}>
